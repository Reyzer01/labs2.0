...
void getMostFrequentLetters(const std::string& text, char mostFrequent[], int count) {
    ...
    // Заполнение массива наиболее частыми буквами
...
std::string processWord(const std::string& word, const char mostFrequent[], int freqCount) {
    if (containsEnoughFrequentLetters(word, mostFrequent, freqCount)) {
        ...
        // Преобразование слова в заглавные буквы
        ...
        if (foundLetters.length() > 1) {
            foundLetters.erase(foundLetters.size() - 2); //  Удаление последней запятой и пробела
        }
        foundLetters += ")";
...
int main() {
    ...
    for (size_t i = 0; i < text.length(); i++) {
        ...
        if (isLetter(text[i])) {
            ...
            processedText += text[i]; // Добавление символов, которые не являются буквами
        }
    }
